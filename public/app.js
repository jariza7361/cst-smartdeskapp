// public/app.js — deep-audit baseline + working UI + test fetch
(function () {
  ...
  function showToast(msg, type='success'){
    ...
    t.textContent = msg; t.className = ''; t.classList.add(type); requestAnimationFrame(()=>t.classList.add('show'));
  ...
  $('#t_fetch_vzw_pdf')?.addEventListener('click', async ()=>{
    ...
    const log = $('#testLog'); log.textContent = 'Fetching… '+url;
    try{
  ...
  function runQA(){
    ...
    const dot=$('#qaDot'); if(dot) dot.className='dot '+(misses.length? 'bad' : 'ok');
