...
function startSplash() {
  ...
  setTimeout(() => {
    clearInterval(t);
    finish();
    logQA('Splash forced complete (timeout)');
  }, 5000);
...
function onSaveWizard() {
  ...
  const profile = {
    ...
    id: $('#f_id').value.trim(),
    ext: $('#f_ext').value.trim(),
    coach: $('#f_coach').value.trim()
  };
  if (!s.name || !s.empId || !s.ext) return; // native required also guards
  saveSettings(s);
  if ($('#f_nosplash')?.checked) {
    localStorage.setItem('cst_nosplash', '1');
  } else {
    localStorage.removeItem('cst_nosplash');
  }
...
async function onDrop(e) {
  ...
  if (file.type === 'text/plain') {
    ...
    preview({
      error: 'PDF parsing stub — add pdf.js (self-hosted) later.'
    });
  } else {
    preview({
